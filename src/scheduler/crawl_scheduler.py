...
class CrawlScheduler:
    ...
    def _process_group(self, group_config):
        ...
        try:
            ...
            self.notifier.send_message_sync(
                text=f"❌ Error processing group {group_id}: {str(e)}",
                thread_id=group_config.get_thread_id('error'),
                parse_mode='HTML'
            )
    ...
    def _process_home_topics(self, crawler, group_config):
        ...
        if topics:
            ...
            if success_count > 0:
                ...
                logger.info(f"Successfully forwarded {success_count} home updates for group {group_id}")
        else:
            logger.info(f"No new home content for group {group_id}")
    ...
    def _process_digest_topics(self, crawler: ZsxqCrawler, group_config: GroupConfig):
        ...
        if digest_topics:
            ...
            if success_count > 0:
                ...
                logger.info(f"Successfully forwarded {success_count} digest updates for group {group_id}")
        else:
            logger.info(f"No new digest content for group {group_id}")
